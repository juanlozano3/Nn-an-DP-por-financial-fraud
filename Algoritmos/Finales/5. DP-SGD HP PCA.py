...
def main():
    ...
    def choose_pca_components(X, target_variance=0.95, min_components=10, max_components=300):
        ...
        n_components = np.searchsorted(cum_var, target_variance) + 1
        n_components = max(min_components, n_components)
        n_components = min(max_components, n_components)
        n_components = min(n_components, X.shape[1])
        retained_var = cum_var[n_components - 1] if n_components > 0 else 0.0
        return n_components, retained_var

    target_variance = float(os.environ.get("DP_HP_PCA_TARGET_VAR", 0.95))
    ...
    print(
        f"  • PCA: seleccionando {n_components} componentes (varianza acumulada {retained_var:.4f})"
    )
    ...
    with mlflow.start_run(run_name="DP-SGD-PCA-HPO"):
        ...
        mlflow.log_param("pca_components", int(n_components))
        mlflow.log_metric("pca_variance_retained", float(retained_var))
        mlflow.log_param("pca_target_variance", target_variance)
    ...
    trial_num = 0
    print("\n" + "=" * 80)
    print("INICIANDO BÚSQUEDA DE HIPERPARÁMETROS (PCA)")
    ...
    print("\n" + "=" * 80)
    print("MEJOR MODELO ENCONTRADO (DP-SGD PCA)")
    print("=" * 80)
    ...
    print(f"  • Componentes PCA: {n_components} (varianza {retained_var:.4f})")
