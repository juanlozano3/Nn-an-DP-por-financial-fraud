...
def main():
    ...
    X_majority_downsampled, y_majority_downsampled = resample(
        ...
        n_samples=2000,  # Igualar al número de la clase minoritaria
    ...
    X_minority_downsampled, y_minority_downsampled = resample(
        ...
        y_minority,
        replace=False,  # sin reemplazo
        n_samples=2000,  # <-- también 2000
        random_state=42,
    )

    # 4. Concatenar para obtener el set balanceado
    X_train_balanced = pd.concat([X_majority_downsampled, X_minority_downsampled])
    y_train_balanced = pd.concat([y_majority_downsampled, y_minority_downsampled])
